...
PerseusLD.query_text_annotations  = function(a_query_elem) {  
    ...
    // TODO we will want to separate this out by annotation/collection type 
    // TODO sort by ?date
...
PerseusLD.filter_text_annotations = function(a_elem,a_results) { 
    var cts_work = $("#cts-work-uri a");
    var cts_text = $("#cts-text-uri a");
    var work_uri = PerseusLD.strip_uri_prefix(cts_work.attr("href"));
    var text_uri = PerseusLD.strip_uri_prefix(cts_text.attr("href"));
    var text_uri_regex = "^" + text_uri + "$";
    var work_uri_regex = "^" + work_uri + "$";
    var version_passage_start = null;
    var version_passage_next = null;
    var annotations = { "work": [], "text" : [], "passage" : []};
    var cts_passage = $('#cts-passage-uri a');
    var cts_passage_next = $('#cts-passage-next-uri a');
    
    // extract the starting passage of the displayed text version
    if (cts_passage.length > 0) {
        var passage_uri = PerseusLD.strip_uri_prefix(cts_passage.attr("href"));
        var passage_regex = new RegExp("^" + text_uri + ":(.+)$");
        var passage_match = passage_regex.exec(passage_uri);
        if (passage_match != null) {
            version_passage_start = passage_match[1];
        }
    }
    // extract the next passage of the displayed text version
    if (cts_passage_next.length > 0) {
        var passage_uri = PerseusLD.strip_uri_prefix(cts_passage_next.attr("href"));
        var passage_regex = new RegExp("^" + text_uri + ":(.+)$");
        var passage_match = passage_regex.exec(passage_uri);
        if (passage_match != null) {
            version_passage_next = passage_match[1];
        }
    } 
		  ...
		  if (target.match(new RegExp(work_uri_regex)) != null) {
              ...
              if (target_passage != null) {
                ...
                if (target_passage_start >= version_passage_start &&
                        (version_passage_next == null || target_passage_end < version_passage_next)) {
...
PerseusLD.show_annotations = function(a_type,a_elem,a_start) {
    ...
    $(".results",a_elem).addClass("loading");
    $(a_elem).show();
        	...
        	$.ajax(PerseusLD.results[a_type][i]+"/oac",
       	    {
       	        ...
       	        PerseusLD.transform_annotation(a_data,$(".results",a_elem),this.xhrFields.data.last,this.xhrFields.data.next);
       	            
       	            
       	    }).fail(
       	        function(a_req) { 
       	            PerseusLD.fail_annotation($(".results",a_elem),a_req.data.last)
